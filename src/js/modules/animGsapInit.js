...
export function animGsapInit() {
    ...
    if (sessionStorage.getItem('animationsInitialized')) {
        // Если анимации уже инициализированы, запускаем сразу
        initializeAnimations();
    } else {
        // Первый запуск - устанавливаем флаг
        sessionStorage.setItem('animationsInitialized', 'true');
        
        // Создаем анимации сразу, но отключаем ScrollTrigger
        disableScrollTrigger();
        initializeAnimations();
        
        // Через 3 секунды включаем ScrollTrigger
        window.addEventListener('load', function() {
            setTimeout(() => {
                enableScrollTrigger();
                ScrollTrigger.refresh();
    ...
    function disableScrollTrigger() {
        // Временно отключаем глобально ScrollTrigger
    ...
    function enableScrollTrigger() {
        // Включаем обратно ScrollTrigger
        ScrollTrigger.config({
    ...
    function initializeAnimations() {
        ...
        // Если анимации уже инициализированы, обновляем сразу
        if (sessionStorage.getItem('animationsInitialized')) {
            ScrollTrigger.refresh();
        }
    }
    ...
    function footerAnimation() {
        ...
        const tlFoot = gsap.timeline({
            scrollTrigger: {
                trigger: footerContent,
                start: "bottom 100%",
                end: `bottom+=${accentBoxHeight}px 100%`,
                scrub: true,
                markers: false,
            }
        });
