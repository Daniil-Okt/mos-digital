...
export function maskTel() {
  window.addEventListener("DOMContentLoaded", function() {
      [].forEach.call(document.querySelectorAll('.tel'), function(input) {
          var keyCode;
          var isFirstFocus = true; // Флаг для отслеживания первого фокуса
          
          function mask(event) {
              // Если событие focus и поле пустое и это первый фокус
              if (event.type === "focus" && input.value === "" && isFirstFocus) {
                  isFirstFocus = false;
                  
                  // Устанавливаем маску с задержкой
                  setTimeout(() => {
                      input.value = "+7 (";
              ...
              // Стандартная логика маски для всех остальных случаев
              event.keyCode && (keyCode = event.keyCode);
              var pos = this.selectionStart;
              if (pos < 3) event.preventDefault();
              var matrix = "+7 (___) ___-__-__",
              ...
              if (event.type == "blur" && this.value.length < 5) {
                  this.value = "";
                  isFirstFocus = true; // Сбрасываем флаг при очистке поля
              }
          }
