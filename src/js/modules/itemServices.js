...
export function itemServices() {
    ...
    // Функция для прокрутки к элементу на мобильных устройствах
    function scrollToItem(item) {
        if (window.innerWidth >= 1024) return;
        
        // Ждем немного, чтобы элемент успел раскрыться и изменить высоту
        setTimeout(() => {
            const itemRect = item.getBoundingClientRect();
            const itemTop = itemRect.top + window.pageYOffset;
            
            // Высота header (если есть) плюс небольшой отступ
        ...
        }, 450); // Небольшая задержка для применения CSS-анимаций
    }
